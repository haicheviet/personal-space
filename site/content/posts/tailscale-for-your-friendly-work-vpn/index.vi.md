...
Hiện tại, môi trường làm việc của công ty yêu cầu ruy cập máy chủ và database service từ Bastion Host và VPN. Việc thiết lập ban đầu rất đơn giản, nhưng khi đội ngũ và service phát triển, độ phức tạp của việc thiết lập sẽ tăng lên rất nhiều lần. Onboard người mới không bao giờ dễ dàng và mặc dù mình đã thiết lập [Dev microVM](https://haicheviet.com/cluster-vm-with-firecracker/) với tất cả các environment và package cần thiết, cấu hình mạng vẫn khó hiểu và phức tạp để debug cho junior dev. Các vấn đề của quy trình hiện tại có thể được liệt kê như sau:
